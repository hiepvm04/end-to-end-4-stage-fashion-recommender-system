...
class HopsworksQueryModel:
    ...
    def register(self, mr, feature_view, query_df) -> None:
        # Đường dẫn ổ C an toàn khỏi lỗi Window path
        import os
        safe_path = r"C:\temp\hopsworks_query_model"
        if os.path.exists(safe_path): import shutil; shutil.rmtree(safe_path)
        os.makedirs(os.path.join(safe_path, "variables"), exist_ok=True)
        
        local_model_path = self.save_to_local(safe_path)
    ...
    def deploy(cls, ranking_model_type: Literal["ranking", "llmranking"] = "ranking"):
        ...
        transformer_script_path = f"/Projects/{project.name}/{uploaded_file_path}"
    ...
    def _prepare_secrets(cls, ranking_model_type: Literal["ranking", "llmranking"]):
        project = hopsworks.login(
            host="eu-west.cloud.hopsworks.ai", # <--- THÊM DÒNG NÀY 
...
class HopsworksCandidateModel:
    ...
    def register(self, mr, feature_view, item_df) -> None:
        import os
        safe_path = r"C:\temp\hopsworks_item_model"
        if os.path.exists(safe_path): import shutil; shutil.rmtree(safe_path)
        os.makedirs(os.path.join(safe_path, "variables"), exist_ok=True)
